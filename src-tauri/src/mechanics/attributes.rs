...
use crate::models::AttributeRegistryEntry;
...
fn find_exact_match(conn: &PooledConn, proposed_name: &str) -> AppResult<Option<AttributeRegistryEntry>> {
...
fn load_registry_for_kind(conn: &PooledConn, entity_kind: &str) -> AppResult<Vec<AttributeRegistryEntry>> {
...
fn add_alias(conn: &PooledConn, attribute_id: &str, alias: &str) -> AppResult<()> {
    let current: String = conn.query_row("SELECT aliases_json FROM attribute_registry WHERE id = ?1", [attribute_id], |r| r.get(0))?;
    ...
    if !aliases.iter().any(|a| a.eq_ignore_ascii_case(alias)) {
        ...
        conn.execute("UPDATE attribute_registry SET aliases_json = ?1 WHERE id = ?2", rusqlite::params![updated, attribute_id])?;
    }
    Ok(())
}

fn mint_new(conn: &PooledConn, proposed_name: &str, entity_kind: &str, story_id: &str) -> AppResult<AttributeRegistryEntry> {
...
pub async fn resolve_or_create_attribute(
    ...
    if let Some(entry) = { let conn = pool.get()?; find_exact_match(&conn, proposed_name)? } {
        return Ok(entry);
    }

    let candidates = { let conn = pool.get()?; load_registry_for_kind(&conn, entity_kind)? };
...
pub fn get_or_init_entity_attribute(conn: &PooledConn, entity_id: &str, attribute: &AttributeRegistryEntry) -> AppResult<f64> {
...
pub fn apply_attribute_delta(
    ...
    let max_step = if dramatic { range } else { range * NON_DRAMATIC_MAX_FRACTION };
