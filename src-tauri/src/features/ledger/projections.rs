...
pub fn replay_entities(
    ...
    for event in repository::list_logical_entries(conn, story_id)? {
        ...
        match event.kind.as_str() {
            kind::ENTITY_CREATED => {
                ...
                let anchor = event
                    ...
                    .and_then(|v| v.as_str());
                conn.execute("INSERT INTO story_entity_state (story_id, entity_id, name, appearance_anchor, is_present, updated_at, last_event_id)
                    VALUES (?1, ?2, ?3, ?4, 1, ?5, ?6)
                    ON CONFLICT(story_id, entity_id) DO UPDATE SET name=excluded.name, appearance_anchor=excluded.appearance_anchor, is_present=1, updated_at=excluded.updated_at, last_event_id=excluded.last_event_id",
                    rusqlite::params![story_id, entity_id, name, anchor, now, event.id])?;
            }
            kind::ENTITY_UPDATED => {
                ...
                if let Some(name) = name {
                    conn.execute("UPDATE story_entity_state SET name=?1, appearance_anchor=?2, is_present=1, updated_at=?3, last_event_id=?4 WHERE story_id=?5 AND entity_id=?6",
                        rusqlite::params![name, anchor, now, event.id, story_id, entity_id])?;
                }
            }
            kind::ENTITY_DELETED => {
                if let Some(entity_id) = entity_id {
                    conn.execute("UPDATE story_entity_state SET is_present=0, updated_at=?1, last_event_id=?2 WHERE story_id=?3 AND entity_id=?4",
                    rusqlite::params![now, event.id, story_id, entity_id])?;
                }
            }
            kind::ENTITY_ATTRIBUTE_CHANGED => {
                ...
                conn.execute("INSERT INTO entity_attributes (story_id, entity_id, attribute_id, value, source, updated_at, last_event_id)
                    ...
                    ON CONFLICT(story_id, entity_id, attribute_id) DO UPDATE SET value=excluded.value, source=excluded.source, updated_at=excluded.updated_at, last_event_id=excluded.last_event_id",
                    rusqlite::params![story_id, entity_id, attribute_id, value, source, now, event.id])?;
            }
