...
where
    F: FnOnce(AppHandle, String, Candidate) -> Fut + Send + 'static,
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if !action.is_player {
            emit_error(
                &app,
                &sid,
                &AppError::Other("the narration history does not end with an action turn".into()),
            );
            return;
        }
        ...
        match result {
            Ok((visible, thoughts)) => {
                ...
                if let Err(error) = on_candidate(app.clone(), sid.clone(), candidate).await {
                    emit_error(&app, &sid, &error);
                }
            }
            Err(error) => emit_error(&app, &sid, &error),
        }
    });
    stream_id
}
