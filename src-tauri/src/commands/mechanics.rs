...
use crate::mechanics::pipeline::DiceMode;
use crate::models::{AttributeRegistryEntry, EntityAttributeValue, Roll, RollDetail};
...
#[tauri::command]
pub fn get_story_mechanics_settings(pool: State<Pool>, story_id: String) -> AppResult<MechanicsSettings> {
    let settings = read_story_settings_json(&pool, &story_id)?;
    let dice_mode = settings.get("dice_mode").and_then(|v| v.as_str()).unwrap_or("classifier").to_string();
    let attributes_enabled = settings.get("attributes_enabled").and_then(|v| v.as_bool()).unwrap_or(true);
    Ok(MechanicsSettings { dice_mode: DiceMode::from_str_or_default(&dice_mode).as_str().to_string(), attributes_enabled })
}

#[tauri::command]
pub fn save_story_mechanics_settings(pool: State<Pool>, story_id: String, dice_mode: String, attributes_enabled: bool) -> AppResult<()> {
...
fn list_entity_attributes_sync(conn: &rusqlite::Connection, entity_id: &str) -> AppResult<Vec<EntityAttributeValue>> {
...
#[tauri::command]
pub fn list_entity_attributes(pool: State<Pool>, entity_id: String) -> AppResult<Vec<EntityAttributeValue>> {
    let conn = pool.get()?;
...
pub fn list_rolls_for_passage(pool: State<Pool>, passage_id: String) -> AppResult<Vec<Roll>> {
    let conn = pool.get()?;
    let mut stmt = conn.prepare(&format!("SELECT {ROLL_COLUMNS} FROM rolls WHERE passage_id = ?1 ORDER BY created_at ASC"))?;
...
fn get_entity_name(conn: &rusqlite::Connection, entity_id: &str) -> AppResult<String> {
    conn.query_row("SELECT name FROM entities WHERE id = ?1", [entity_id], |r| r.get(0))
...
fn get_attribute_name(conn: &rusqlite::Connection, attribute_id: &str) -> AppResult<String> {
    conn.query_row("SELECT canonical_name FROM attribute_registry WHERE id = ?1", [attribute_id], |r| r.get(0))
...
pub fn get_roll_detail(pool: State<Pool>, passage_id: String) -> AppResult<Option<RollDetail>> {
    ...
    Ok(Some(RollDetail { roll, actor_name, actor_attribute_name, target_name, target_attribute_name, actor_attributes, target_attributes }))
}
