...
pub(crate) async fn generate_for_passage(
    ...
    let text_config = crate::commands::passages::resolve_text_model(app, pool)?;
...
pub(crate) fn maybe_auto_image(app: &AppHandle, pool: &Pool, passage_id: &str, narrated_text: &str) {
    let Ok(settings) = settings::read_image_model_settings(app, pool) else { return };
    ...
    tauri::async_runtime::spawn(async move {
        let prompt = format!("You just wrote this passage:\n\n{narrated_text}\n\nShould it be illustrated?");
        let Ok(decision) = narrator::prompt_typed::<ImageDecision>(&config, IMAGE_DECISION_PREAMBLE, prompt).await
        else {
            return;
        };
