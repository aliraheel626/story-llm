...
pub fn init_pool(app_data_dir: &Path) -> AppResult<Pool> {
    ...
    let pool = r2d2::Pool::new(manager).map_err(crate::error::AppError::Pool)?;
...
fn add_column_if_missing(conn: &PooledConn, table: &str, column: &str, decl_type: &str) -> AppResult<()> {
    let mut stmt = conn.prepare(&format!("PRAGMA table_info({table})"))?;
    let existing: Vec<String> = stmt.query_map([], |row| row.get::<_, String>(1))?.collect::<Result<Vec<_>, _>>()?;
